...
myKeys c =
  ...
  , ("M-M1-l", addName "Lock" $ spawn "slock")
  -- , ("M-S-p", addName "Screen" $ spawn "shiftToNext")
  -- , ("M--p", addName "Shift to Next Screen"   $ shiftNextScreen)  -- Di chuyển cửa sổ sang màn hình tiếp theo
  , ("M-S-p", addName "Shift to Previous Screen" $ shiftPrevScreen)

  -- Screenshot
  , ("M-<Print>", addName "Screenshot" $ spawn "gnome-screenshot -f ~/Pictures/Screenshots/screenshot_$(date +%Y-%m-%d_%H-%M-%S).png")
  , ("M-S-<Print>", addName "Screenshot Selection" $ spawn "gnome-screenshot -a -f ~/Pictures/Screenshots/screenshot_$(date +%Y-%m-%d_%H-%M-%S).png")
  ...
  --- Dmenu 
  , ("M-S-<Return>", addName "Dmenu" $ spawn "dmenu_run -fn 'JetBrainsMono Nerd Font:size=10' -nb '#300924' -nf '#FFFFFF' -sb '#3465A4' -sf '#FFFFFF' -p '🔍 Dmenu:'")]
